      ...
      <section className="pt-32 pb-12">
        <div className="mx-auto max-w-3xl px-5 sm:px-8">
          ...
          <p className="text-gray-1 text-lg max-w-2xl">
            ...
            you’ll get a real answer, usually the same day.
          </p>
        </div>
      </section>
      ...
      <section className="pb-24">
        <div className="mx-auto max-w-3xl px-5 sm:px-8">
          <ul className="divide-y divide-white/10 border-t border-b border-white/10">
            {rows.map((r) => (
              <li key={r.route} className="grid grid-cols-1 sm:grid-cols-[120px_1fr_auto] gap-3 sm:gap-6 py-5 items-baseline">
                <div className="display text-lg">{r.route}</div>
                <div className="text-sm text-gray-1">{r.use}</div>
                <a href={`mailto:${r.address}`} className="text-sm underline underline-offset-4 text-white hover:text-offwhite">
                  {r.address}
                </a>
              </li>
            ))}
